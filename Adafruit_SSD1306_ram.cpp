...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  ram = &Ram;
  ram_address = 0;
  const uint32_t clkSpeed = 20000000UL;
  ram->begin(clkSpeed);
#endif
  clearDisplay();
  if (HEIGHT > 32) {
    drawBitmap((WIDTH - splash_width) / 2, (HEIGHT - splash_height) / 2,
               Splash, splash_width, splash_height, 1);
  }
  /*
  else {
...
void Adafruit_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
 if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    ...
    case SSD1306_INVERSE:
      ...
      break;
#else
    uint32_t adr = x + (y / 8) * WIDTH;
    uint8_t val = ram->readByte(adr);
    switch (color) {
    case SSD1306_WHITE:
      ram->writeByte(adr, (const uint8_t) (val | (1 << (y & 7))));
      break;
    case SSD1306_BLACK:
      ram->writeByte(adr, (const uint8_t) (val & ~(1 << (y & 7))));
      break;
...
#else
  uint8_t byte = 0;
  for (uint32_t a_ptr = 0; a_ptr < (uint32_t) (WIDTH * ((HEIGHT + 7) / 8)); ++a_ptr) {
    ram->writeByte(a_ptr + ram_address, byte);
    
    uint8_t get_byte = ram->readByte(a_ptr + ram_address);
    if (get_byte != byte) {
...
#else
      uint32_t a_ptr = ((y / 8) * WIDTH + x);
      uint8_t val = ram->readByte(a_ptr + ram_address);
      
      switch (color) {
      case SSD1306_WHITE:
        while (w--) {
          ram->writeByte(a_ptr + ram_address, (const uint8_t) (val | mask));
        };
        break;
      case SSD1306_BLACK:
        mask = ~mask;
          ...
          if (color == SSD1306_INVERSE) {
...
#else
              ram->writeByte(a_ptr + ram_address, (const uint8_t) (val ^ 0xFF));
              a_ptr += WIDTH;
#endif
            ...
            do {
              ...
              *pBuf = val;   // Set byte
              pBuf += WIDTH; // Advance pointer 8 rows
#else
              ram->writeByte(a_ptr + ram_address, (const uint8_t) (val));
              a_ptr += WIDTH;
#endif
              h -= 8;        // Subtract 8 rows from height
            } while (h >= 8);
...
#else
    return (ram->readByte(x + (y / 8) * WIDTH) & (1 << (y & 7)));
#endif
  }
...
void Adafruit_SSD1306::display(void) {
  ...
  if (wire) { // I2C
    ...
    while (count--) {
      ...
      WIRE_WRITE(*ptr++);
#else
      get_byte = ram->readByte(a_ptr + ram_address);
      WIRE_WRITE(get_byte);
      a_ptr++;
#endif
      bytesOut++;
